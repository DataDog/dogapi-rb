...
You want to track a new metric called `some.metric.name` and have just sampled it from `my_device` on `my_host`. Its value is 50.
Here is how you submit the value to Datadog.
...
data point you will need to pass a list of `Time`, `float` pairs, instead of a simple `float` value.
 ...
 dog.emit_points 'some.metric.name', [[t1, 50.0], [t2, 51.0], [t3, -60.]], host => "my_host", device => "my_device"
